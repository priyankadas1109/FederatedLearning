...
federated_datasets = random_split(train_dataset, split_sizes)
...
class SkinCancerClient(fl.client.NumPyClient):
    def __init__(self, dataset):
        ...
        self.model = SkinCancerCNN().to(DEVICE)  # Move to GPU if available
    ...
    def fit(self, parameters, config):
        ...
        return self.get_parameters(config), len(self.dataset), {"loss": avg_loss}

    def evaluate(self, parameters, config):
        self.set_parameters(parameters)
        test_loader = DataLoader(self.dataset, batch_size=32, shuffle=False)
        ...
        return loss_total / total, total, {"accuracy": accuracy}



# Create client function (FIXED)
def client_fn(cid):
    return SkinCancerClient(federated_datasets[int(cid)])
...
class CustomFedAvg(fl.server.strategy.FedAvg):
    def __init__(self, **kwargs):
        ...
        self.round_client_losses: Dict[int, List[float]] = {}
    ...
    def aggregate_fit(
        ...
        self.round_client_losses[server_round] = losses
...
client_model = SkinCancerClient(test_dataset)  # test_dataset already defined
...
with torch.no_grad():
    for images, labels in test_loader:
        ...
        y_score.extend(outputs.cpu().numpy())  # logits
...
# Binarize labels (One-vs-Rest for ROC)
...
# Add each class curve
for i in range(7):
    fig_roc.add_trace(go.Scatter(
        x=fpr[i], y=tpr[i],
        mode='lines',
        name=f"{list(label_mapping.keys())[i]} (AUC = {roc_auc[i]:.2f})",
        hovertemplate=f"<b>{list(label_mapping.keys())[i]}</b><br>FPR: %{{x:.2f}}<br>TPR: %{{y:.2f}}"
    ))

# Add diagonal reference (chance) line
fig_roc.add_trace(go.Scatter(
    x=[0, 1], y=[0, 1],
    mode='lines',
    name='Chance',
...
fig_roc.update_layout(
    title="📊 Multi-class ROC Curve (Interactive)",
...
rounds = sorted(strategy.round_client_losses.keys())
loss_matrix = [strategy.round_client_losses[r] for r in rounds]
acc_matrix = [strategy.round_client_accuracies[r] for r in rounds]

loss_matrix = np.array(loss_matrix)
acc_matrix = np.array(acc_matrix)
clients = list(range(loss_matrix.shape[1]))

print("\n Debugging: Round-wise Loss and Accuracy Matrices")

print("\n Rounds:", rounds)
print("\n Loss Matrix (Rounds x Clients):")
print(loss_matrix)

print("\n Accuracy Matrix (Rounds x Clients):")
print(acc_matrix)

if loss_matrix.size == 0 or acc_matrix.size == 0:
    print("\n Warning: One or both matrices are empty. Please check `strategy.round_client_losses` and `strategy.round_client_accuracies` population logic.")
else:
    print("\n Matrices loaded successfully with shape:")
    print("Loss Matrix shape:", loss_matrix.shape)
    print("Accuracy Matrix shape:", acc_matrix.shape)

# LOSS Plot
fig_loss = go.Figure(data=[go.Surface(
    z=loss_matrix,
    x=clients,
    y=rounds,
    colorscale='Viridis',
...
fig_loss.update_layout(
    title=" Loss Surface over Rounds & Clients",
    scene=dict(xaxis_title='Client', yaxis_title='Round', zaxis_title='Loss'),
...
# ACCURACY Plot
# Shape check
print("acc_matrix shape:", acc_matrix.shape)  # Should be (5, 5)

fig_acc = go.Figure(data=[go.Surface(
    z=acc_matrix,
    x=np.arange(acc_matrix.shape[1]),  # clients
    y=np.arange(acc_matrix.shape[0]),  # rounds
    colorscale='YlGnBu',
...
fig_acc.update_layout(
    title='Accuracy Surface over Rounds & Clients',
    title_font_size=20,
    scene=dict(
        xaxis_title='Client',
        yaxis_title='Round',
